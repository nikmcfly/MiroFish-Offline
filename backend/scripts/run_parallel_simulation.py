...
def disable_oasis_logging():
    """
    Disable detailed OASIS library log output
    OASIS logging is too verbose (logs each agent's observations and actions), we use our own action_logger
    """
...
def init_logging_for_simulation(simulation_dir: str):
    ...
    """
    # Disable detailed OASIS logging
...
class ParallelIPCHandler:
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Those needing to interview on both platforms
...
def fetch_new_actions_from_db(
    ...
    """
    Get new action records from database and supplement with complete context info
    
    Args:
        db_path: Database file path
        last_rowid: Last read max rowid value (using rowid instead of created_at, because created_at formats differ across platforms)
...
def _enrich_action_context(
    ...
    """
    Supplement context info for action (post content, username, etc.)
...
async def run_twitter_simulation(
    ...
    for round_num in range(total_rounds):
        ...
        # Get actually executed actions from database and record them
...
async def main():
    ...
    # Create shutdown event at the start of main to ensure entire program can respond to exit signals
...
def setup_signal_handlers(loop=None):
    ...
    2. Give program a chance to clean up resources (close database, environment, etc.)
    3. Then exit
    """
