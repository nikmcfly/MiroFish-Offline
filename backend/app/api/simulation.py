...
def get_graph_entities(graph_id: str):
    ...
    Only returns nodes matching predefined entity types (nodes with labels other than just Entity)
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # - preparing: If config_generated=True it means already completed
        # - running: Currently running, which means preparation was completed long ago
...
def prepare_simulation():
    ...
    Steps:
    1. Check if completed preparation work already exists
    ...
    try:
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Completed preparation work exists, no need for duplicate generation",
        ...
        # This allows the frontend to get the expected Agent count immediately after calling prepare
        try:
...
def get_prepare_status():
    ...
    2. Check via simulation_id if completed preparation work already exists
    ...
    Response:
        {
            ...
            "data": {
                ...
                "progress": 45,
                "message": "...",
                "already_prepared": true|false,  // Whether already has completed preparation
    ...
    try:
            ...
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "Completed preparation work already exists",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
        ...
        if not task:
            # Task not found, but if simulation_id exists, check whether preparation is complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def get_simulation_history():
    ...
    For homepage historical project display, returns simulation list with rich info like project name, description, etc.
    ...
    try:
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use user-set total_rounds, otherwise use recommended rounds
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    try:
        ...
        # Check if generating (by checking state.json)
        is_generating = False
        total_expected = None
...
def start_simulation():
    ...
    About the force parameter:
        ...
        - Will not clean up config files (simulation_config.json) and profile files
        - Suitable for scenarios requiring simulation re-run

    About enable_graph_memory_update:
        - When enabled, all Agent activities (posting, commenting, liking, etc.) during simulation will be updated to Zep graph in real-time
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": f"Simulation is running. Please call /stop endpoint to stop first, or use force=true to force restart"
                            }), 400
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions only shows the latest round from both platforms
...
def get_agent_stats(simulation_id: str):
    ...
    For frontend Agent activity ranking, action distribution display, etc.
    """
    try:
