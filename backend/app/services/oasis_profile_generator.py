...
Converts entities from the graph into Agent Profile format required by OASIS simulation platform

Optimizations:
1. Calls graph retrieval to further enrich node info
...
class OasisProfileGenerator:
    ...
    Converts graph entities into Agent Profiles required for OASIS simulation
    
    Optimized features:
    1. Calls graph retrieval for richer context
    ...
    # Group/institution entity types (need representative account persona)
    GROUP_ENTITY_TYPES = [
    ...
    def _search_graph_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use GraphStorage hybrid search to get rich entity-related info
        ...
        try:
            ...
            logger.info(f"Graph hybrid search complete: {entity_name}, retrieved {len(results['facts'])} facts, {len(results['node_summaries'])} related nodes")
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        Includes:
        1. Edge info from the entity itself (facts)
        2. Detailed info of associated nodes
        3. Rich info from graph hybrid retrieval
        """
        context_parts = []
        ...
        # 4. Use graph hybrid retrieval for richer info
    ...
    def _is_individual_entity(self, entity_type: str) -> bool:
        """Determine if entity is individual type"""
        return entity_type.lower() in self.INDIVIDUAL_ENTITY_TYPES
    
    def _is_group_entity(self, entity_type: str) -> bool:
        """Determine if entity is group/institution type"""
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user persona generation expert. Generate detailed, realistic personas for opinion simulation, maximally restoring existing real-world conditions. Must return valid JSON format. All string values must not contain unescaped newlines. Always respond in English."
        return base_prompt
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        """Build group/institution entity detailed persona prompt"""
...
2. persona: Detailed account profile description (2000 characters of plain text), should include:
   ...
   - Institutional memory (important part of persona: introduce this institution's connection to the event, and its existing actions and reactions in the event)
3. age: Fixed at 30 (virtual age for institutional accounts)
4. gender: Fixed "other" (institutional accounts use other to indicate non-individual)
...
- Institutional account speech must match its identity positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                logger.error(f"Persona generation failed for entity {entity.name}: {str(e)}")
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Required fields:
        - user_id: User ID (integer, used for matching poster_agent_id in initial_posts)
        - username: Username
        - name: Display name
        - bio: Bio
