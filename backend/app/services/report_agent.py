...
Generates simulation reports using ReACT pattern (via GraphStorage / Neo4j)

Features:
1. Generate reports based on simulation requirements and graph info
...
4. Supports conversation with user, autonomously calling retrieval tools during conversation
"""
...
class ReportLogger:
    ...
    Generates agent_log.jsonl file in report folder, recording every detailed action step.
...
This is not LLM simulation — it calls the real interview interface to get original responses from simulated Agents.
...
PLAN_SYSTEM_PROMPT = """\
You are an expert in writing "Future Prediction Reports," possessing a "God's perspective" over the simulated world — you can observe every Agent's behavior, statements, and interactions within the simulation.
...
- The statements and actions of Agents in the simulated world are predictions of future population behavior
...
Please examine this future rehearsal from "God's perspective":
...
1. [Must Call Tools to Observe the Simulated World]
   - You are observing a rehearsal of the future from "God's perspective"
...
3. [Language Consistency - Quoted Content Must Be Translated to Report Language]
   ...
   - When quoting content returned by tools in a different language, you must translate it into fluent report language before writing it into the report
...
- The section title is automatically added by the system; you only need to write plain body content
...
Weibo, as the first scene of public opinion, served as the core platform for initial information dissemination:
...
- panorama_search: Wide-angle panoramic search, understand the full picture of events, timelines, and evolution process
...
class ReportAgent:
    ...
    1. Planning stage: analyze simulation requirements, plan report outline structure
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "insight_forge":
                ...
                # In-depth interview - calls real OASIS interview API to get simulated Agent responses (dual platform)
    ...
    def _generate_section_react(
        ...
        4. Repeat until info sufficient or max iterations reached
            ...
            if has_tool_calls and has_final_answer:
                ...
                if conflict_retries <= 2:
                    ...
                    messages.append({
                        "role": "user",
                        "content": (
                            "[Format Error] Your response included both a tool call and Final Answer, which is not allowed.\n"
    ...
    def generate_report(
        ...
        # If no report_id passed, auto-generate
        if not report_id:
    ...
    def chat(
        ...
        for iteration in range(max_iterations):
            ...
            tool_results = []
            for call in tool_calls[:1]:  # Max 1 tool call execution per round
...
class ReportManager:
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Because the section title is added by the system, content should not have any headings
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        3. Clean excess empty lines and separator lines
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check for duplicate title (same content heading within 5 consecutive lines)
                is_duplicate = False
                ...
                if is_duplicate:
                    # Skip duplicate title and trailing empty lines
                    i += 1
                ...
                if level == 1:
                    if title == outline.title:
                        ...
                        # Convert other level-1 titles to bold
                        processed_lines.append(f"**{title}**")
                    ...
                    if title in section_titles or title == outline.title:
                        ...
                        # Convert non-section level-2 titles to bold
                        processed_lines.append(f"**{title}**")
                        processed_lines.append("")
                        prev_was_heading = False
                else:
