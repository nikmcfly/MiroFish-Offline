      ...
      <div class="header-center">
        <div class="view-switcher">
          <button 
            ...
            @click="viewMode = mode"
          >
            {{ { graph: 'Graph', split: 'Geteilt', workbench: 'Arbeitsfläche' }[mode] }}
          </button>
        </div>
      </div>
...
// Schritt-Zustand
const currentStep = ref(1) // 1: Graphaufbau, 2: Umgebung, 3: Simulation, 4: Bericht, 5: Interaktion
const stepNames = ['Graphaufbau', 'Umgebung einrichten', 'Simulation', 'Bericht', 'Interaktion']
...
const handleNewProject = async () => {
  ...
  try {
    ...
    addLog('Ontologie-Generierung wird gestartet: Dateien werden hochgeladen...')
...
const startGraphPolling = () => {
  addLog('Abfrage von Graphdaten gestartet...')
  fetchGraphData()
...
const loadGraph = async (graphId) => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(`Fehler beim Laden der Graphdaten: ${res.error}`)
    }
  } catch (e) {
